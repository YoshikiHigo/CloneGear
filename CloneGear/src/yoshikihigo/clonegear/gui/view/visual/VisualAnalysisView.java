...
public class VisualAnalysisView extends JPanel implements ViewScale,
	...
	public VisualAnalysisView(final int width, final int height) {
		...
		fileSelectionPane.setBorder(new TitledBorder(
				new LineBorder(Color.black), "File Selection Panel"));
		final JSplitPane fileListPane = new JSplitPane(
				JSplitPane.VERTICAL_SPLIT, true);
		final JSplitPane directoryTreePane = new JSplitPane(
				JSplitPane.VERTICAL_SPLIT, true);
		...
		this.horizontalFileListView.getScrollPane().setBorder(
				new TitledBorder(new LineBorder(Color.black), "Horizontal"));
		SelectedEntities.<GUIFile> getInstance(HORIZONTAL_FILE).addObserver(
				this.horizontalFileListView);
		...
		this.verticalFileListView.getScrollPane().setBorder(
				new TitledBorder(new LineBorder(Color.black), "Vertical"));
		SelectedEntities.<GUIFile> getInstance(VERTICAL_FILE).addObserver(
				this.verticalFileListView);
		RNRValue.getInstance(RNR).addObserver(this.verticalFileListView);
		UninterestingClonesDisplay.getInstance(UNINTERESTING).addObserver(
				this.verticalFileListView);

		this.horizontalDirectoryTreeView = new DirectoryTreeView(
				DIRECTION.HORIZONTAL);
		this.horizontalDirectoryTreeView.getScrollPane().setBorder(
				new TitledBorder(new LineBorder(Color.black), "Horizontal"));
		SelectedEntities.<GUIFile> getInstance(HORIZONTAL_FILE).addObserver(
				this.horizontalDirectoryTreeView);
		RNRValue.getInstance(RNR).addObserver(this.horizontalDirectoryTreeView);
		UninterestingClonesDisplay.getInstance(UNINTERESTING).addObserver(
				this.horizontalDirectoryTreeView);

		this.verticalDirectoryTreeView = new DirectoryTreeView(
				DIRECTION.VERTICAL);
		this.verticalDirectoryTreeView.getScrollPane().setBorder(
				new TitledBorder(new LineBorder(Color.black), "Vertical"));
		SelectedEntities.<GUIFile> getInstance(VERTICAL_FILE).addObserver(
				this.verticalDirectoryTreeView);
		RNRValue.getInstance(RNR).addObserver(this.verticalDirectoryTreeView);
		UninterestingClonesDisplay.getInstance(UNINTERESTING).addObserver(
				this.verticalDirectoryTreeView);

		this.horizontalSourceCodeWindow = new SourceCodeWindow(
				DIRECTION.HORIZONTAL);
		SelectedEntities.<GUIFile> getInstance(HORIZONTAL_FILE).addObserver(
				this.horizontalSourceCodeWindow);
		SelectedEntities.<GUIFile> getInstance(VERTICAL_FILE).addObserver(
				this.horizontalSourceCodeWindow);
		SelectedEntities.<GUIClonePair> getInstance(CLONEPAIR).addObserver(
				this.horizontalSourceCodeWindow);
		...
		SelectedEntities.<GUIFile> getInstance(HORIZONTAL_FILE).addObserver(
				this.horizontalFilePathView);
		this.horizontalFilePathView.setBorder(new TitledBorder(new LineBorder(
				java.awt.Color.black), "Horizontal File Path"));

		this.verticalFilePathView = new FilePathView();
		SelectedEntities.<GUIFile> getInstance(VERTICAL_FILE).addObserver(
				this.verticalFilePathView);
		this.verticalFilePathView.setBorder(new TitledBorder(new LineBorder(
				java.awt.Color.black), "Vertical File Path"));
		...
		fileListPane.setTopComponent(this.horizontalFileListView
				.getScrollPane());
		fileListPane.setBottomComponent(this.verticalFileListView
				.getScrollPane());

		directoryTreePane.setTopComponent(this.horizontalDirectoryTreeView
				.getScrollPane());
		directoryTreePane.setBottomComponent(this.verticalDirectoryTreeView
				.getScrollPane());
		...
		this.rightSplitPane.setRightComponent(this.sourceSplitPane);
		...
		this.sourceSplitPane.setBorder(new TitledBorder(new LineBorder(
				Color.black), "Source Code View"));

		horizontalFilePanel.add(this.horizontalSourceCodeWindow.scrollPane,
				BorderLayout.CENTER);
		horizontalFilePanel
				.add(this.horizontalFilePathView, BorderLayout.NORTH);
		verticalFilePanel.add(this.verticalSourceCodeWindow.scrollPane,
				BorderLayout.CENTER);
	...
	public void showClonePairList() {
		if (null == this.rightSplitPane.getLeftComponent()) {
			this.rightSplitPane
					.setLeftComponent(this.clonePairListView.scrollPane);
		}
		this.rightSplitPane
				.setLastDividerLocation(VISUAL_CLONEPAIRLISTVIEW_MIN_WIDTH);
	}
